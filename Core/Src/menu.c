...
void menu_game_options(game_options_t *options, snes_controller_t *controller) {
	...
	while(!done) {
		...
		if (controller->current_button_state != controller->previous_button_state && controller->current_button_state) {
			...
			} else if (controller->current_button_state & SNES_START_MASK || controller->current_button_state & SNES_A_MASK) {
				done = 1;
			}
		}
